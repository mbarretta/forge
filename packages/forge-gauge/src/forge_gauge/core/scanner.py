...
from forge_gauge.core.retry_queue import RetryQueue
from forge_gauge.utils.docker_utils import DockerClient
from forge_gauge.utils.chps_utils import CHPSScanner
...
class VulnerabilityScanner:
    ...
    def __init__(
        ...
        Args:
            ...
            organization_mode: Whether scanning from --organization flag. When True,
                enables chainguard-private fallback for Chainguard images.
        """
        self.cache = cache
    ...
    def scan_image(
        ...
        if self.check_fresh_images:
            # Enable chainguard-private fallback only for Chainguard images in organization mode
            allow_cg_private_fallback = (
                self.organization_mode and context == "chainguard"
            )
            image, used_fallback, pull_successful, error_type = self.docker.ensure_fresh_image(
                image, self.platform, upstream_image=upstream_image,
                allow_chainguard_private_fallback=allow_cg_private_fallback,
            )
            if used_fallback:
                logger.warning(
    ...
    def scan_image_pair(
        ...
        try:
            ...
            except (TimeoutError, Exception) as partial_err:
                logger.debug(f"Could not retrieve alternative scan result: {partial_err}")

            try:
                if 'cg_future' in locals():
                    cg_result = cg_future.result(timeout=0)
            except (TimeoutError, Exception) as partial_err:
                logger.debug(f"Could not retrieve chainguard scan result: {partial_err}")
    ...
    def _update_result_with_analysis(
        ...
        """
        Update a ScanResult with new analysis for the specified context.

        Args:
            result: The ScanResult to update
            analysis: The new ImageAnalysis to apply
            context: Either "alternative" or "chainguard"
                ...
                try:
                    ...
                    if failed_pull.pair_index is not None:
                        ...
                        if result_idx is not None:
                            ...
                            results[result_idx] = self._update_result_with_analysis(
                                result, analysis, failed_pull.context
                            )
