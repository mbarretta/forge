...
from forge_gauge.constants import CATALOG_CACHE_TTL_SECONDS, CLI_SUBPROCESS_TIMEOUT, DEFAULT_LLM_CONFIDENCE
...
class LLMMatcher:
    ...
    # Patterns indicating transient failures that should not be cached
    ...
    def _is_transient_failure(self, reasoning: str) -> bool:
        """
        Check if the reasoning indicates a transient failure that should not be cached.

        Args:
            reasoning: LLM reasoning string

        Returns:
            True if this is a transient failure, False otherwise
        """
        reasoning_lower = reasoning.lower()
        return any(pattern in reasoning_lower for pattern in self.TRANSIENT_FAILURE_PATTERNS)
    ...
    def _cache_result(
        ...
        Skips caching for transient failures (catalog unavailable, API errors, etc.)
        ...
        """
        # Don't cache transient failures - they should be retried
        if not chainguard_image and self._is_transient_failure(reasoning):
            logger.debug(f"Skipping cache for transient failure: {reasoning}")
            return
    ...
    def _load_full_catalog(self, org: str = "chainguard-private") -> list[str]:
        ...
        if catalog_cache_file.exists():
            try:
                ...
                if time.time() - cache_time < CATALOG_CACHE_TTL_SECONDS:
...
If you cannot find information, say "Unknown image".""",
    ...
    def _generate_search_terms(self, base_name: str, org_name: Optional[str] = None) -> list[str]:
        ...
        if len(words) > 1:
            ...
            # Note: "node" is excluded because it's ambiguous (Node.js vs infrastructure node)
            # "driver", "registrar", "csi" are too generic for CSI components
            generic_words = (
                "manager", "controller", "server", "client", "k8s", "kubernetes",
                "node", "driver", "registrar", "csi", "plugin", "sidecar",
            )
...
**IMPORTANT - Avoid false positives from partial name matches:**
- "node" is Node.js runtime - do NOT match infrastructure tools containing "node" (like csi-node-driver, calico-node) to it
- "redis" is the Redis database - do NOT match redis-exporter or redis-operator to it
- Match based on what the software DOES, not partial word overlap in names
- CSI drivers should match to kubernetes-csi-* images, not to unrelated images
