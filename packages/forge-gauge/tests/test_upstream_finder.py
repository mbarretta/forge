...
from forge_gauge.utils.upstream_finder import UpstreamImageFinder, UpstreamResult
...
"""
    ...
    def test_base_extraction_python(self, mock_exists, tmp_path):
        ...
        Note: base_extract has a high per-strategy threshold (0.85) by default,
        so we patch it lower to test the extraction logic itself.
        """
        finder = UpstreamImageFinder(
            manual_mappings_file=tmp_path / "nonexistent.yaml",
            min_confidence=0.5,
    ...
    def test_base_extract_filtered_by_per_strategy_threshold(self, mock_exists, tmp_path):
        ...
        Even though base_extract returns 70% confidence and the global min_confidence
        is 70%, the per-strategy threshold for base_extract is 85%, so it gets filtered.
        This is intentional to prevent false positives from heuristic name extraction.
        """
        finder = UpstreamImageFinder(
            manual_mappings_file=tmp_path / "nonexistent.yaml",
            min_confidence=0.7,
        ...
        # Mock: Only python base image verification succeeds (for base_extract strategy)
        # Other strategies (registry_strip, common_registry) should fail
        def mock_verify(image: str) -> bool:
            return "library/python:latest" in image

        mock_exists.side_effect = mock_verify

        result = finder.find_upstream("internal-python-app:v1")

        # Base extraction returns 0.70 confidence, but per-strategy threshold is 0.85
        # So the result should be filtered out
        assert result.upstream_image is None
        assert result.confidence == 0.0
        assert result.method == "none"
    ...
    def test_csi_node_driver_not_matched_to_node(self, mock_exists, tmp_path):
        ...
        This is a regression test for the bug where 'node' appearing in the middle
        of a CSI driver name was incorrectly matched to the Node.js 'node' image.
        """
        finder = UpstreamImageFinder(
            manual_mappings_file=tmp_path / "nonexistent.yaml",
            min_confidence=0.7,
        )
        mock_exists.return_value = True  # Even if node:latest exists, shouldn't match

        result = finder.find_upstream("eks/csi-node-driver-registrar:v2.15.0-eksbuild.3")
    ...
    def test_calico_node_not_matched_to_node(self, mock_exists, tmp_path):
        ...
        Calico's 'node' component is network infrastructure, not Node.js.
        """
        finder = UpstreamImageFinder(
            manual_mappings_file=tmp_path / "nonexistent.yaml",
            min_confidence=0.7,
        ...
        result = finder.find_upstream("calico/node:v3.26.0")

        # calico/node should NOT match node:latest (Node.js)
        # It's a Calico networking component
        assert result.upstream_image is None or result.upstream_image != "node:latest"
